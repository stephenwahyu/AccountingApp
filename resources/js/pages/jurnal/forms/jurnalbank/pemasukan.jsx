...
export default function FormPemasukanBank({ journal = null, accounts = [], periods = [], bankAccounts = [], }) {
  ...
  const [data, setData] = useState({
    ...
    details: journal?.details?.map(d => ({
      ...
      credit: d.credit
    })) || [{ temp_id: generateUniqueId(), account_id: "", description: "", credit: 0 }],
  });
  ...
  const addRow = () => {
    setData(prev => ({...prev, details: [...prev.details, { temp_id: generateUniqueId(), account_id: "", description: "", credit: 0 }]}));
  };
  ...
  const totalCredit = useMemo(
    () => data.details.reduce((sum, detail) => sum + Number.parseFloat(detail.credit || 0), 0),
    [data.details]
  );
  ...
  const handleSubmit = (statusValue) => {
    ...
    if (!data.bank_account_id) {
        toast.error("Harap pilih akun bank penyetoran.");
        return;
    }
    ...
    router[method](url, submitData, {
        ...
        onError: (errors) => {
            setErrors(errors);
            toast.error("Gagal menyimpan pemasukan bank. Harap periksa kembali inputan Anda.");
            setProcessing(false);
            setSubmittedStatus(null);
        },
    });
          ...
          <div className="flex flex-col gap-6">
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Atur data administratif dan akun bank penerima.</CardDescription>
                </CardHeader>
                <CardContent>
            ...
            <Card className="overflow-hidden border-primary/10 shadow-md">
                <CardHeader className="bg-muted/20 pb-4 border-b">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <CardDescription>Tentukan akun-akun pendapatan atau piutang sebagai sumber dana bank.</CardDescription>
                        </div>
                        <Button
                            type="button"
                            variant="outline"
                ...
                <CardFooter className="flex flex-col sm:flex-row justify-between gap-6 items-stretch sm:items-center bg-muted/30 py-6 px-6 border-t">
                    <div className="flex flex-col gap-1">
                        <p className="text-xs uppercase font-semibold text-muted-foreground">Akun Bank Penerima</p>
