...
export default function FormPengeluaranBank({ journal = null, accounts = [], periods = [], bankAccounts = [], }) {
  ...
  const [data, setData] = useState({
    entry_date: journal?.entry_date ? parseSafeDate(journal.entry_date) : new Date(),
  ...
  const handleSubmit = (statusValue) => {
    ...
    if (!data.bank_account_id) {
        toast.error("Harap pilih akun bank sumber dana.");
        return;
    }
    ...
    const url = isEdit
      ? route("jurnal.bank.pengeluaran.update", journal.id)
      : route("jurnal.bank.pengeluaran.store");
        ...
        <form onSubmit={(e) => e.preventDefault()}>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
            ...
            <div className="flex flex-wrap justify-end gap-2 w-full sm:w-auto">
              ...
              <Button onClick={() => handleSubmit('Draft')} disabled={totalDebit === 0 || !data.bank_account_id || processing} variant="secondary" className="w-full sm:w-auto">
                {processing && submittedStatus === 'Draft' ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Save className="h-4 w-4 mr-2" />}
                Simpan Draft
              </Button>
              <Button onClick={() => handleSubmit('Posted')} disabled={totalDebit === 0 || !data.bank_account_id || processing} className="w-full sm:w-auto">
          ...
          <div className="flex flex-col gap-6">
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Atur data administratif dan akun bank sumber dana.</CardDescription>
                </CardHeader>
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
                        ...
                        <div className="grid gap-2 lg:col-span-2">
                            <div className="flex flex-col gap-3 md:items-center md:flex-row">
                                <div className="grid gap-2">
                            <Label htmlFor="bank_account_id" className="text-xs uppercase tracking-wider text-muted-foreground">Ambil dari Akun Bank</Label>
                            <Select
                        ...
                        <div className="grid gap-2">
                            <Label htmlFor="penerima" className="text-xs uppercase tracking-wider text-muted-foreground">Dibayarkan Kepada</Label>
                            <Input
                                id="penerima"
            ...
            <Card className="overflow-hidden border-primary/10 shadow-md">
                <CardHeader className="bg-muted/20 pb-4 border-b">
                    <div className="flex items-center justify-between">
                        <div>
                            <CardTitle className="text-lg">Tujuan Pengeluaran</CardTitle>
                            <CardDescription>Tentukan akun-akun beban atau liabilitas sebagai tujuan pengeluaran bank.</CardDescription>
                        </div>
                        <Button
                            type="button"
                            variant="outline"
                    ...
                    <div className="overflow-x-auto">
                        <Table>
                            <TableHeader className="bg-muted/30">
                                <TableRow>
                                    <TableHead className="w-[30%] pl-6">Akun Debit</TableHead>
                ...
                <CardFooter className="flex flex-col sm:flex-row justify-between gap-6 items-stretch sm:items-center bg-muted/30 py-6 px-6 border-t">
                    <div className="flex flex-col gap-1">
                        <p className="text-xs uppercase font-semibold text-muted-foreground">Akun Bank Sumber</p>
                        <p className="text-sm font-medium">
                            {data.bank_account_id 
                                ? bankAccountOptions.find(o => o.value === data.bank_account_id)?.label 
                                : "Belum dipilih"}
                        </p>
                    </div>
                    <div className="flex flex-col sm:flex-row gap-8">
                        <div className="text-right">
                            <p className="text-xs uppercase font-semibold text-muted-foreground mb-1">Total Pengeluaran</p>
